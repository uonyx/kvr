    ...
    if (this->get (&curr))
    {
      switch (curr)
      {
        ...
        case MSGPACK_HEADER_STRING_8:   // string8
        {
          ...
          if (success)
          {
            KVR_ASSERT (slen < KVR_CONSTANT_MAX_KEY_LENGTH);
            uint8_t strk [KVR_CONSTANT_MAX_KEY_LENGTH];          
            success = this->get (strk, slen);
            strk [slen] = 0;
            success = ctx.read_string ((const char *) strk, slen);
          }
#endif
...
bool kvr_msgpack::read (kvr::value *dest, kvr::istreambuf &istr)
{
  KVR_ASSERT (dest);
#if 0
  msgpack_reader reader (&istr);
  msgpack_read_context ctx (dest);
