

/*
 * Copyright (c) 2015 Ubaka Onyechi
 * 
 * kvr is free software distributed under the MIT license.
 * See LICENSE file for details.
 */



#define C_IMPL 0

#if C_IMPL
#include "kvr_json.h"
#include "kvr_internal.h"
#include "rapidjson/reader.h"
#include "rapidjson/writer.h"
#include "rapidjson/error/en.h"
#endif

// json_read_context


#if C_IMPL
struct json_read_context
{
  json_read_context (kvr::value *value) : m_root (value), m_temp (NULL), m_depth (0)
  {
    memset (m_stack, 0, sizeof (m_stack));
  }

  ///////////////////////////////////////////
  ///////////////////////////////////////////
  ///////////////////////////////////////////

  bool Null ()
  {
    bool success = true;

    KVR_ASSERT_SAFE (m_depth != 0, success);
    kvr::value *node = m_stack [m_depth - 1];
    KVR_ASSERT (node);
    KVR_ASSERT (node->is_map () || node->is_array ());

    if (node->is_map ())
    {
      KVR_ASSERT_SAFE (m_temp && m_temp->is_null (), false);
      m_temp = NULL;
    }
    else if (node->is_array ())
    {
      node->push_null ();
    }
    else
    {
      success = false;
    }

    return success;
  }

  bool Bool (bool b)
  {
    bool success = true;

    KVR_ASSERT_SAFE (m_depth != 0, success);
    kvr::value *node = m_stack [m_depth - 1];
    KVR_ASSERT (node);
    KVR_ASSERT (node->is_map () || node->is_array ());

    if (node->is_map ())
    {
      KVR_ASSERT_SAFE (m_temp && m_temp->is_null (), false);
#if KVR_OPTIMIZATION_IMPLICIT_TYPE_CONVERSION_OFF
      m_temp->conv_boolean ();
#endif
      m_temp->set_boolean (b);
      m_temp = NULL;
    }
    else if (node->is_array ())
    {
      node->push (b);
    }
    else
    {
      success = false;
    }

    return success;
  }

  bool Int (int i)
  {
    return Int64 ((int64_t) i);
  }

  bool Uint (unsigned u)
  {
    return Int64 ((int64_t) u);
  }

  bool Int64 (int64_t i)
  {
    bool success = true;

    KVR_ASSERT_SAFE (m_depth != 0, success);
    kvr::value *node = m_stack [m_depth - 1];
    KVR_ASSERT (node);
    KVR_ASSERT (node->is_map () || node->is_array ());

    if (node->is_map ())
    {
      KVR_ASSERT_SAFE (m_temp && m_temp->is_null (), false);
#if KVR_OPTIMIZATION_IMPLICIT_TYPE_CONVERSION_OFF
      m_temp->conv_integer ();
#endif
      m_temp->set_integer (i);
      m_temp = NULL;
    }
    else if (node->is_array ())
    {
      node->push (i);
    }
    else
    {
      success = false;
    }

    return success;
  }

  bool Uint64 (uint64_t u)
  {
    KVR_ASSERT (m_depth != 0);
    KVR_ASSERT (false && "not supported");
    KVR_REF_UNUSED (u);
    return false;
  }

  bool Double (double d)
  {
    bool success = true;

    KVR_ASSERT_SAFE (m_depth != 0, success);
    kvr::value *node = m_stack [m_depth - 1];
    KVR_ASSERT (node);
    KVR_ASSERT (node->is_map () || node->is_array ());

    if (node->is_map ())
    {
      KVR_ASSERT_SAFE (m_temp && m_temp->is_null (), false);
#if KVR_OPTIMIZATION_IMPLICIT_TYPE_CONVERSION_OFF
      m_temp->conv_float ();
#endif
      m_temp->set_float (d);
      m_temp = NULL;
    }
    else if (node->is_array ())
    {
      node->push (d);
    }
    else
    {
      success = false;
    }

    return success;
  }

  bool String (const char *str, kvr_rapidjson::SizeType length, bool copy)
  {
    bool success = true;

    KVR_ASSERT_SAFE (m_depth != 0, success);
    KVR_REF_UNUSED (copy);

    kvr::value *node = m_stack [m_depth - 1];
    KVR_ASSERT (node);
    KVR_ASSERT (node->is_map () || node->is_array ());

    if (node->is_map ())
    {
      KVR_ASSERT_SAFE (m_temp && m_temp->is_null (), false);
#if KVR_OPTIMIZATION_IMPLICIT_TYPE_CONVERSION_OFF
      m_temp->conv_string ();
#endif
      m_temp->set_string (str, (kvr::sz_t) length);
      m_temp = NULL;
    }
    else if (node->is_array ())
    {
      kvr::value *vstr = node->push_null (); KVR_ASSERT (vstr);
      vstr->conv_string ();
      vstr->set_string (str, (kvr::sz_t) length);
    }
    else
    {
      success = false;
    }

    return success;
  }

  bool StartObject ()
  {
    bool success = true;
    kvr::value *node = NULL;

    if (m_depth > 0)
    {
      node = m_stack [m_depth - 1];
      KVR_ASSERT (node);
      KVR_ASSERT (node->is_map () || node->is_array ());

      if (node->is_map ())
      {
        node = m_temp;
        KVR_ASSERT_SAFE (m_temp && m_temp->is_null (), false);        
        m_temp->conv_map ();
        m_temp = NULL;
      }
      else if (node->is_array ())
      {
        node = node->push_map ();
      }
      else
      {
        success = false;
      }
    }
    else
    {
      node = m_root->conv_map ();
    }

    KVR_ASSERT (m_depth < KVR_CONSTANT_MAX_TREE_DEPTH);
    m_stack [m_depth++] = node;

    return success;
  }

  bool Key (const char *str, kvr_rapidjson::SizeType length, bool copy)
  {
    kvr::value *node = m_stack [m_depth - 1];
    KVR_ASSERT_SAFE (node && node->is_map (), false);

    KVR_REF_UNUSED (length);
    KVR_REF_UNUSED (copy);

    KVR_ASSERT (!m_temp);
    m_temp = node->insert_null (str);
    return (m_temp != NULL);
  }

  bool EndObject (kvr_rapidjson::SizeType memberCount)
  {
    kvr::value *node = m_stack [m_depth - 1];
    KVR_ASSERT_SAFE (node && node->is_map (), false);
    KVR_ASSERT (node->size () == (kvr::sz_t) memberCount);
    KVR_REF_UNUSED (memberCount);

    m_stack [--m_depth] = NULL;
    return true;
  }

  bool StartArray ()
  {
    bool success = true;
    kvr::value *node = NULL;

    if (m_depth > 0)
    {
      node = m_stack [m_depth - 1];
      KVR_ASSERT (node);
      KVR_ASSERT (node->is_map () || node->is_array ());

      if (node->is_map ())
      {
        node = m_temp;
        KVR_ASSERT_SAFE (m_temp && m_temp->is_null (), false);
        m_temp->conv_array ();        
        m_temp = NULL;
      }
      else if (node->is_array ())
      {
        node = node->push_array ();
      }
      else
      {
        success = false;
      }
    }
    else
    {
      node = m_root->conv_array ();
    }

    KVR_ASSERT (m_depth < KVR_CONSTANT_MAX_TREE_DEPTH);
    m_stack [m_depth++] = node;

    return success;
  }

  bool EndArray (kvr_rapidjson::SizeType elementCount)
  {
    kvr::value *node = m_stack [m_depth - 1];
    KVR_ASSERT_SAFE (node && node->is_array (), false);
    KVR_ASSERT (node->length () == (kvr::sz_t) elementCount);
    KVR_REF_UNUSED (elementCount);
    m_stack [--m_depth] = NULL;
    return true;
  }

  ///////////////////////////////////////////
  ///////////////////////////////////////////
  ///////////////////////////////////////////

  kvr::value  * m_stack [KVR_CONSTANT_MAX_TREE_DEPTH];
  kvr::value  * m_root;
  kvr::value  * m_temp;
  kvr::sz_t     m_depth;
};
#endif



#if C_IMPL
namespace kvr_rapidjson // private
{
  bool write_stream (const kvr::value *val, kvr_rapidjson::Writer<kvr_rapidjson::omemstream_impl> &writer)
  {
    bool success = false;

    //////////////////////////////////
    if (val->is_map ())
    //////////////////////////////////
    {
      bool ok = writer.StartObject ();
      kvr::value::cursor c = val->fcursor ();
      kvr::pair p;
      while (ok && c.get (&p))
      {
        kvr::key *k = p.get_key ();
        ok = writer.Key (k->get_string (), k->get_length ());

        kvr::value *v = p.get_value ();
        ok = ok && write_stream (v, writer);
      }
      success = ok && writer.EndObject ();
    }

    //////////////////////////////////
    else if (val->is_array ())
    //////////////////////////////////
    {
      bool ok = writer.StartArray ();
      for (kvr::sz_t i = 0, c = val->length (); (i < c) && ok; ++i)
      {
        kvr::value *v = val->element (i);
        ok = write_stream (v, writer);
      }
      success = ok && writer.EndArray ();
    }

    //////////////////////////////////
    else if (val->is_string ())
    //////////////////////////////////
    {
      kvr::sz_t slen = 0;
      const char *str = val->get_string (&slen);
      success = writer.String (str, slen);
    }

    //////////////////////////////////
    else if (val->is_integer ())
    //////////////////////////////////
    {
      int64_t n = val->get_integer ();
      success = writer.Int64 (n);
    }

    //////////////////////////////////
    else if (val->is_float ())
    //////////////////////////////////
    {
      double n = val->get_float ();
      success = writer.Double (n);
    }

    //////////////////////////////////
    else if (val->is_boolean ())
    //////////////////////////////////
    {
      bool b = val->get_boolean ();
      success = writer.Bool (b);
    }

    //////////////////////////////////
    else if (val->is_null ())
    //////////////////////////////////
    {
      success = writer.Null ();
    }

    return success;
  }
}
#endif

// kvr_json


#if C_IMPL
bool kvr_rapidjson::read (kvr::value *dest, kvr::istream &istr)
{
  KVR_ASSERT (dest);
  
  bool success = false;

  const char *str = (const char *) istr.bytes ();
  KVR_ASSERT (str);
  size_t len = 0; KVR_REF_UNUSED (len); //KVR_ASSERT (len > 0); 
  
  json_read_context rctx (dest);
  kvr_rapidjson::StringStream ss (str); 
  kvr_rapidjson::Reader reader;

  kvr_rapidjson::ParseResult ok = reader.Parse (ss, rctx);
#if KVR_DEBUG
  if (ok.IsError ()) { std::fprintf (stderr, "JSON parse error: %s (%lu)", kvr_rapidjson::GetParseError_En (ok.Code ()), ok.Offset ()); }
#endif
  success = ok && (rctx.m_depth == 0);

  return success;
}



bool kvr_rapidjson::read (kvr::value *dest, kvr::istreambuf &istr)
{
  KVR_ASSERT (dest);
  bool success = false;

#if 1
  json_read_context rctx (dest);
  kvr_rapidjson::icustream_impl ss (&istr);

  kvr_rapidjson::Reader reader;
  kvr_rapidjson::ParseResult ok = reader.Parse (ss, rctx);
#if KVR_DEBUG
  if (ok.IsError ()) { std::fprintf (stderr, "JSON parse error: %s (%lu)", kvr_rapidjson::GetParseError_En (ok.Code ()), ok.Offset ()); }
#endif
  success = ok && (rctx.m_depth == 0);
#else
  KVR_REF_UNUSED (istr);
#endif

  return success;
}



bool kvr_rapidjson::write (const kvr::value *src, kvr::ostream *ostr)
{
  KVR_ASSERT (ostr);

  bool success = false;
  
  kvr_rapidjson::omemstream_impl wctx (ostr);
  kvr_rapidjson::Writer<kvr_rapidjson::omemstream_impl> writer (wctx);
  success = kvr_rapidjson::write_stream (src, writer);
  KVR_REF_UNUSED (src);

  ostr->set_eos (0);

  return success;
}



bool kvr_rapidjson::write (const kvr::value *src, kvr::ostreambuf *ostr)
{
  KVR_ASSERT (ostr);

  bool success = false;

  kvr_rapidjson::ocustream_impl wctx (ostr);
  kvr_rapidjson::Writer<kvr_rapidjson::ocustream_impl> writer (wctx);
  //success = json_write_context::write_stream (src, writer);
  KVR_REF_UNUSED (src);

  return success;
}
#endif


#if C_IMPL
size_t kvr_rapidjson::write_approx_size (const kvr::value *val)
{
  size_t size = 0;

  //////////////////////////////////
  if (val->is_map ())
  //////////////////////////////////
  {
    size += 2; // brackets
    kvr::value::cursor c = val->fcursor ();
    kvr::pair p;
    while (c.get (&p))
    {
      kvr::key *k = p.get_key ();
      kvr::value *v = p.get_value ();

      size += k->get_length () + 2; // + quotes
      size += write_approx_size (v);
      size += 2; // colon and comma
    }
  }

  //////////////////////////////////
  else if (val->is_array ())
  //////////////////////////////////
  {
    size += 2; // brackets
    for (kvr::sz_t i = 0, c = val->length (); i < c; ++i)
    {
      kvr::value *v = val->element (i);
      size += kvr_internal::ndigitsu32 (i);
      size += write_approx_size (v);
      size += 1; // comma
    }
  }

  //////////////////////////////////
  else if (val->is_string ())
  //////////////////////////////////
  {
    kvr::sz_t slen = 0;
    const char *str = val->get_string (&slen);
    KVR_REF_UNUSED (str);
    size += (slen + 2); // + quotes
  }

  //////////////////////////////////
  else if (val->is_integer ())
  //////////////////////////////////
  {
    int64_t n = val->get_integer ();
    size += kvr_internal::ndigitsi64 (n);
  }

  //////////////////////////////////
  else if (val->is_float ())
  //////////////////////////////////
  {
    //size += 13; // average (guess)     
    size += 25; // 25 is max
  }

  //////////////////////////////////
  else if (val->is_boolean ())
  //////////////////////////////////
  {
    bool b = val->get_boolean ();
    size += b ? 4 : 5;
  }

  //////////////////////////////////
  else if (val->is_null ())
  //////////////////////////////////
  {
    size += 4;
  }

  return size;
}
#endif

