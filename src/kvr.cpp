...
void kvr::ostream::put (uint8_t byte)
{
  KVR_ASSERT (m_bytes);
...
void kvr::ostream::put (uint8_t *bytes, size_t count)
{
#if 0 // no best-write
  size_t c = kvr_internal::min ((m_cap - m_pos), count);
  memcpy (&m_bytes [m_pos], bytes, c);
  m_pos += c;
  return c;
#endif
...
void kvr::ostream::pop (size_t count)
{
  m_pos -= count;
}
